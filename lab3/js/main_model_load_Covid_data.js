...
class MainModelCovidData {
	/**
	 * MainModelCovidData constructor.
	 */
	constructor(csvCovidCodebookData, csvCovidData, csvVaccinationsData, topologyData) { 
		this.covidCodebookDictionary = {};
		this.covidData = {};
		this.vaccinationData = {};
		this.countryContinentData = {};
		this.worldTopologyData = topologyData;
		this.readCovidCodebookDictionaryData(csvCovidCodebookData,this.covidCodebookDictionary);
		this.readCovidData(csvCovidData, this.countryContinentData, this.covidData);
		this.readVaccinationsData(csvVaccinationsData, this.countryContinentData, this.vaccinationData);
		this.readWorldTopologyData(topologyData, this.worldTopologyData );
	}

	readCovidCodebookDictionaryData(csvCovidCodebookData,covidCodebookDictionary) {
	...
	readCovidData(csvCovidData, countryContinentData, covidData) {
	...
	readVaccinationsData(csvVaccinationsData, countryContinentData, vaccinationData) {
	...
	getAllContinentsLatestCovidData() {
		...
		for (let continentIndex in continents){
			allContinentLatestCovidData.push(this.covidData[continents[continentIndex]].latestRecord);
		}
		return allContinentLatestCovidData;
	}

	getAllContinentCountriesLatestCovidData(continent) {
		let allContinentCountriesLatestCovidData = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent == continent) {
				allContinentCountriesLatestCovidData.push(this.covidData[location].latestRecord);
			}
		}
		return allContinentCountriesLatestCovidData;
	}

	getAllContinentCountriesLatestVaccinationData(continent) {
		let allContinentCountriesLatestVaccinationData = [];
		for (let location in this.vaccinationData){
			if(this.vaccinationData[location].latestRecord.continent == continent) {
				allContinentCountriesLatestVaccinationData.push(this.vaccinationData[location].latestRecord);
			}
		}
		return allContinentCountriesLatestVaccinationData;
	}

	getContinentTop10CountriesCasesWithLatestCovidData(continent) {
		let data = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent == continent) {
				data.push(this.covidData[location].latestRecord);
			}
		}
	...
	getContinentTop10CountriesDeathsWithLatestCovidData(continent) {
		let data = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent == continent) {
				data.push(this.covidData[location].latestRecord);
	...
	getAllCountriesLatestCovidData() {
		let allCountriesLatestCovidData = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent) {
				allCountriesLatestCovidData.push(this.covidData[location].latestRecord);
			}
		}	
		return allCountriesLatestCovidData;
	}

	getWorldCovidAndTotalVaccinationsData() {
		let allWorldCovidData = [];
		for (let index in this.covidData["World"].allRecords){
			if(this.covidData["World"].allRecords[index].total_vaccinations > 0) {
				allWorldCovidData.push(this.covidData["World"].allRecords[index]);
			}
		}	
		return allWorldCovidData;
	}

	getWorldCovidAndTotalBoostersData() {
		let allWorldCovidData = [];
		for (let index in this.covidData["World"].allRecords){
			if(this.covidData["World"].allRecords[index].total_boosters > 0) {
				allWorldCovidData.push(this.covidData["World"].allRecords[index]);
			}
		}	
		return allWorldCovidData;
	...
	getAllCountriesLatestCovidAndHandwashingFacilitiesData() {
		let allCountriesLatestCovidData = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent && this.covidData[location].latestRecord.handwashing_facilities > 0) {
				allCountriesLatestCovidData.push(this.covidData[location].latestRecord);
			}
		}	
		return allCountriesLatestCovidData;
	...
	getAllCountriesLatestCovidAndDevIndexData() {
		let allCountriesLatestCovidData = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent && this.covidData[location].latestRecord.human_development_index > 0) {
				allCountriesLatestCovidData.push(this.covidData[location].latestRecord);
			}
		}	
		return allCountriesLatestCovidData;
	...
	getAllCountriesLatestCovidAndExtremePovertyData() {
		let allCountriesLatestCovidData = [];
		for (let location in this.covidData){
			if(this.covidData[location].latestRecord.continent && this.covidData[location].latestRecord.extreme_poverty > 0) {
				allCountriesLatestCovidData.push(this.covidData[location].latestRecord);
			}
		}	
		return allCountriesLatestCovidData;
	...
	getAllCountriesLatestVaccinationData() {
		let allCountriesLatestVaccinationData = [];
		for (let location in this.vaccinationData){
			if(this.vaccinationData[location].latestRecord.continent) {
				allCountriesLatestVaccinationData.push(this.vaccinationData[location].latestRecord);
			}
		}	
		return allCountriesLatestVaccinationData;
	}

	getAllLocationsLatestCovidData() {
		let allLocationsLatestCovidData = [];
		for (let location in this.covidData){
			allLocationsLatestCovidData.push(this.covidData[location].latestRecord);
		}
		return allLocationsLatestCovidData;
	}

	getAllLocationCovidData(location) {
		return this.covidData[location].allRecords;
	}

	getAllLocationVaccinationData(location) {
		return this.vaccinationData[location].allRecords;
	}

	getLatestLocationCovidData(location) {
		return this.covidData[location].latestRecord;
	}

	getLocationContinentName(location) {
		return this.covidData[location].latestRecord.continent;
	}
	...
	convertToNumber(data, MakeNullZero) {
		if (data) {
			return Number(data);
		}
		else if (MakeNullZero) {
	...
	readWorldTopologyData(topologyData, worldTopologyData) {
